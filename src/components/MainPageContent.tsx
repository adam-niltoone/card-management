  ...
  return (
    <div>
      ...
      {selectedNote ? (
        ...
        <button type="submit" className="bg-blue-700 hover:bg-blue-800 text-white rounded-lg p-2">
          Add Card
        </button>
      )}
      ...
      {/* Dropdown button */}
        ...
        {/* Dropdown menu */}
        <div 
            className={`z-10 ${dropdownOpen ? 'block' : 'hidden'} bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 mt-2`}
            aria-labelledby="dropdownDefaultButton">
            <ul className="py-2 text-sm text-gray-700 dark:text-gray-200">
                <li>
                <Link to="/demo-drag-drop" className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                    Demo Drag and Drop
                </Link>
                </li>
                <li>
                  <a href="#" className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">Back</a>
                </li>    
            </ul>
        </div>
    </form>
        ...
        {notes.map((note) => (
          ...
          >
            <div className="flex justify-end">
              <button onClick={(event) => deleteNote(event, note.id)} className="text-xl">
                x
              </button>
            </div>
            <h2 className="text-xl pb-2 font-bold text-left">{note.title}</h2>
